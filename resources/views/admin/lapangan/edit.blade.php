...
<div class="max-w-6xl mx-auto pb-10">
    ...
    <!-- FORM SECTIONS -->
    <form id="lapangan-form" method="POST" action="{{ route('admin.lapangan.update', $pengaturan->id) }}" enctype="multipart/form-data" novalidate>
        @csrf
        ...
        <div class="step-section active" id="step-1">
            <div class="bg-white rounded-2xl shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] border border-gray-100 p-5 md:p-8 mb-6">
                ...
                <div class="mb-6">
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Nama Lapangan <span class="text-red-500">*</span></label>
                    <input type="text" name="nama_arena" required class="form-input w-full px-4 py-3 rounded-xl border border-gray-200 text-gray-800 transition-all bg-gray-50 focus:bg-white" value="{{ old('nama_arena', $pengaturan->nama_arena) }}">
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <input type="text" name="jenis_olahraga" required class="form-input w-full px-4 py-3 rounded-xl border border-gray-200 text-gray-800 transition-all bg-gray-50 focus:bg-white" value="{{ old('jenis_olahraga', $pengaturan->jenis_olahraga ?? 'Badminton') }}" placeholder="Ketik Cabang Olahraga (contoh: Badminton, Futsal, Basket...)">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Deskripsi Singkat (Kartu) <span class="text-red-500">*</span></label>
                ...
                <div class="mb-6">
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Gambar Arena (Muncul di Halaman Pemesanan)</label>
                    @if(isset($pengaturan) && $pengaturan->gambar_utama)
                        <div class="mb-3">
                            <img src="{{ asset('storage/' . $pengaturan->gambar_utama) }}" alt="Gambar Utama" class="w-48 h-auto rounded-lg shadow-sm border border-gray-200">
                        </div>
                    @endif
                    <div class="relative">
                        <input type="file" name="gambar_utama" id="gambar_utama_input" class="hidden" accept="image/*" onchange="document.getElementById('gambar_utama_filename').textContent = this.files[0] ? this.files[0].name : 'Belum ada file terpilih'">
                        <label for="gambar_utama_input" class="flex items-center gap-3 px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 hover:bg-white cursor-pointer transition-all">
                            <span class="px-4 py-1.5 bg-gray-200 hover:bg-gray-300 rounded-lg text-xs font-bold text-gray-700">Pilih File</span>
                            <span id="gambar_utama_filename" class="text-sm text-gray-500 truncate">Belum ada file terpilih</span>
                        </label>
                    </div>
                    <p class="text-xs text-gray-500 mt-2">Biarkan kosong jika tidak ingin mengubah gambar.</p>
                </div>

                <!-- CATATAN PRICELIST & KEBIJAKAN KHUSUS CABOR INI -->
                <div class="p-5 bg-gradient-to-br from-blue-50/70 to-indigo-50/50 rounded-2xl border border-blue-200/80">
                    <div class="flex items-center gap-3 mb-3">
                        ...
                        <div>
                            <label class="block text-sm font-bold text-gray-800">Catatan Pricelist & Kebijakan Pemesanan (Khusus Cabang Ini)</label>
                            <p class="text-xs text-gray-500">Teks ini akan muncul saat pengunjung mengklik icon/tab cabor ini di modal pricelist beranda.</p>
                        </div>
                    </div>
                    <textarea name="catatan_member" rows="5" class="form-input w-full px-4 py-3 rounded-xl border border-gray-200 text-gray-800 transition-all bg-white focus:ring-2 focus:ring-blue-500 text-sm leading-relaxed" placeholder="Contoh:
1. Waktu Pembayaran: Batas waktu pembayaran adalah 10 menit setelah checkout.
2. Verifikasi QRIS: Pastikan mengunggah bukti transfer yang jelas.
3. E-Tiket: Tunjukkan QR Code saat kedatangan.
4. Paket Member: Berlaku untuk 4 pekan rutin pada hari & jam yang sama.">{{ old('catatan_member', $pengaturan->catatan_member) }}</textarea>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] border border-gray-100 p-5 md:p-8 mb-6">
                ...
                <div class="mb-6">
                    ...
                    <div class="flex flex-wrap gap-3">
                        @php 
                            $fasilitas = ['Kipas', 'Parkiran', 'Kantin', 'Wifi', 'Musholla', 'Toilet', 'Ruang Ganti']; 
            ...
            <div class="bg-white rounded-2xl shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] border border-gray-100 p-5 md:p-8 mb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="prefix_lapangan" required placeholder="Contoh: Lapangan, Karpet, Meja" class="form-input w-full px-4 py-3 rounded-xl border border-gray-200 text-gray-800 transition-all bg-gray-50 focus:bg-white" value="{{ old('prefix_lapangan', $pengaturan->prefix_lapangan ?? 'Lapangan') }}">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1.5">Jumlah Lapangan <span class="text-red-500">*</span></label>
                        <input type="number" min="1" max="100" name="jumlah_lapangan" required placeholder="Masukkan jumlah lapangan" class="form-input w-full px-4 py-3 rounded-xl border border-gray-200 text-gray-800 transition-all bg-gray-50 focus:bg-white" value="{{ old('jumlah_lapangan', $pengaturan->jumlah_lapangan ?? 4) }}">
                    </div>
                    <div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="harga_per_jam" required class="form-input w-full pl-11 pr-4 py-3 rounded-xl border border-gray-200 text-gray-800 transition-all bg-gray-50 focus:bg-white" value="{{ number_format((float)preg_replace('/[^0-9]/', '', old('harga_per_jam', $pengaturan->harga_per_jam ?? 80000)), 0, ',', '.') }}">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] border border-gray-100 p-5 md:p-8 mb-6">
                ...
                <label class="flex items-center gap-4 bg-blue-50/50 border border-blue-100 p-4 rounded-xl mb-6 cursor-pointer hover:bg-blue-50 transition-colors">
                    <div class="relative flex items-center justify-center">
                        <input type="checkbox" name="is_member_active" id="is_member_active_checkbox" onchange="toggleMemberFields()" class="peer appearance-none w-6 h-6 border-2 border-blue-400 rounded bg-white checked:bg-blue-600 checked:border-blue-600 transition-all cursor-pointer" {{ (old('is_member_active', $pengaturan->is_member_active ?? true)) ? 'checked' : '' }}>
                ...
                <div id="member-fields-container" class="transition-all duration-300">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <div>
                            ...
                            <div class="flex items-center w-full">
                                <input type="number" min="1" max="52" name="member_jumlah_pekan" id="input_member_jumlah_pekan" class="form-input flex-1 min-w-0 px-4 py-3 rounded-l-xl border border-r-0 border-gray-200 text-gray-800 transition-all bg-gray-50 focus:bg-white font-medium text-sm" value="{{ old('member_jumlah_pekan', $pengaturan->member_jumlah_pekan ?? 4) }}">
                        ...
                        <div>
                            ...
                            <div class="flex items-center w-full">
                                <input type="number" min="1" max="24" name="member_jam_per_pertemuan" id="input_member_jam_per_pertemuan" class="form-input flex-1 min-w-0 px-4 py-3 rounded-l-xl border border-r-0 border-gray-200 text-gray-800 transition-all bg-gray-50 focus:bg-white font-medium text-sm" value="{{ old('member_jam_per_pertemuan', $pengaturan->member_jam_per_pertemuan ?? 2) }}">
                                <span class="px-4 py-3 bg-gray-100 border border-gray-200 text-gray-600 font-semibold text-sm rounded-r-xl select-none flex items-center justify-center whitespace-nowrap">
                                    Jam
                                </span>
                            </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <span class="absolute left-4 top-3.5 text-gray-500 font-medium">Rp</span>
                            <input type="text" name="member_harga" id="input_member_harga" class="form-input w-full pl-11 pr-4 py-3 rounded-xl border border-gray-200 text-gray-800 transition-all bg-gray-50 focus:bg-white" value="{{ number_format((float)preg_replace('/[^0-9]/', '', old('member_harga', $pengaturan->member_harga ?? 1000000)), 0, ',', '.') }}">
                        </div>
                    </div>
                </div>
            </div>
