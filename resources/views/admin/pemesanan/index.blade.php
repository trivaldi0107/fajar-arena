...
@section('content')
    ...
    <div class="flex flex-col xl:flex-row gap-4 justify-between items-start xl:items-center mb-8">
        ...
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full xl:w-auto">
            ...
            <button type="button" onclick="triggerDirectAudioPicker()" class="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold text-sm shadow-md shadow-blue-500/20 transition flex items-center justify-center gap-2 cursor-pointer shrink-0 w-full sm:w-auto" title="Pilih File Suara Notifikasi dari Perangkat">
                ...
                <td class="py-4 px-4 sm:px-5 text-right whitespace-nowrap">
                    <div class="flex items-center justify-end gap-2">
                        ...
                        @if($item->canBeDeleted())
                            <button type="button" onclick="openDeleteModal('{{ route('admin.pemesanan.destroy', $item->id) }}', '{{ $item->kode_reservasi }}')" class="px-2.5 py-1.5 bg-rose-50 text-rose-600 hover:bg-rose-600 hover:text-white rounded-xl text-xs font-bold transition border border-rose-200 hover:border-rose-600 flex items-center gap-1 cursor-pointer" title="Hapus Pemesanan (Sudah lewat 24 jam dari waktu main)">
